...
task :deploy do
	on roles(:web) do
		...
		execute "mkdir -p #{deploy_to}/current/static/jidoris"
		execute "rm -rf #{deploy_to}/current/static/products"
		execute "mkdir -p #{deploy_to}/current/static/products"
		execute "mkdir -p #{deploy_to}/current/static/outputs"
